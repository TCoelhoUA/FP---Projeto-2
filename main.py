'''
INFORMAÇÕES:
Leia o README.txt por favor

A minha apiKey: 041fa32a7f874d2594bd27b29a1a39fd (também está mais à frente no código)
'''

# Módulos importados
import requests
import time # Este módulo é utilizado para poder controlar quando é que a próxima linha de código é executada.
import json # Este módulo é utilizado para ler informação da API.
from prettytable import PrettyTable # Este módulo é utilizado para criar tabelas com informação.
import csv # Este módulo é utilizado para exportar dados em .csv
import shutil # Este módulo é utilizado apenas para centar o conteúdo dos prints através da função .center(shutil.get_terminal_size().columns).
...
def menu_options():
    ...
    if choice == "1":
        print(program_details.center(shutil.get_terminal_size().columns))
...
def define_inputs():
    lon = input("\nInsira a sua longitude.\n=>  ")

    while not is_number(lon.replace(".","")):
        print("\n\u26A0 A longitude introduza é inválida! \u26A0".center(shutil.get_terminal_size().columns))
        lon = input("Insira a sua longitude:\n=>  ")

    lat = input("\nInsira a sua latitude:\n=>  ")

    while not is_number(lat.replace(".","")):
        print("\n\u26A0 A latitude introduza é inválida! \u26A0".center(shutil.get_terminal_size().columns))
        lat = input("Insira a sua latitude:\n=>  ")

    radius = input("\nQuão longe quer viajar? (em km)\n=>  ")

    while not is_number(radius):
        print("\n\u26A0 A distância introduzida é inválida! \u26A0".center(shutil.get_terminal_size().columns))
...
def show_main_categories(categories_python_file):
    for category in categories_python_file:
        ...
        if "." not in category:
            print(f"\u2022 {category}".center(shutil.get_terminal_size().columns))
...
def define_categories(categories_python_file):
    ...
    print("Insira as suas categorias de interesse (termine inserindo um input vazio)".center(shutil.get_terminal_size().columns))
    ...
    while appended_category not in categories_python_file:
        if appended_category == "":
            print("\u26A0 Tem de escolher pelo menos uma categoria! \u26A0".center(shutil.get_terminal_size().columns))
        else:
            print("\u26A0 Tem de escolher uma categoria válida! \u26A0".center(shutil.get_terminal_size().columns))
    ...
    while appended_category != "":
        ...
        while check_subcategory(appended_category, final_category, categories_python_file):
            print("\nSubcategorias:".center(shutil.get_terminal_size().columns))
            subcategories = []
            ...
            for item in subcategories:
                print(f"\u2022 {item}".center(shutil.get_terminal_size().columns))
...
def csv_export(total_attractions, formatted_data, average_distance):
    ...
    with open(f"{file_name}.csv", "w", encoding="utf-8") as fileobj:
        ...
        # Adiciona a primeira e a segunda linha do ficheiro CSV com (--- Detalhes ---) e os nomes dos dados que serão escritos nas linhas seguintes.
        writer.writerow(["--- Detalhes ---"])
        writer.writerow(["Nome", "País", "Distrito", "Concelho", "Freguesia", "Latitude", "Longitude", "Distância (m)"])
        ...
        for attraction in range(len(formatted_data)):
            ...
            writer.writerow([property["name"], property["country"], property["county"], property["city"], property["district"], property["lat"], property["lon"], property["distance"]/1000])

        # Adiciona as estatísticas finais ao ficheiro CSV
        writer.writerow("")
        writer.writerow(["--- Estatísticas ---"])
        writer.writerow(["Total de Atrações", "Distância Média (km)"])
        writer.writerow([total_attractions, average_distance/1000])

    # Informa o utilizador de que o ficheiro foi criado com sucesso.
    print(f"O ficheiro {file_name}.csv foi criado com sucesso na pasta do projeto.".center(shutil.get_terminal_size().columns))
...
def store_and_format_data(data_dict):
    ...
    if total_attractions == 0:
        print("\n\u26A0  Não foram encontradas atrações que cumpram os requisitos introduzidos. \u26A0".center(shutil.get_terminal_size().columns))
...
def main():
    print(welcome.center(shutil.get_terminal_size().columns))
